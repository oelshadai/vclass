"""
Phase 4: Backend Academic Enforcement Models
All academic rules enforced at database and model level
"""
...
class Assignment(models.Model):
    """Assignment with strict academic enforcement"""
    ...
    # Time constraints (enforced at model level)
    start_date = models.DateTimeField(default=timezone.now)
    due_date = models.DateTimeField()
    time_limit_minutes = models.PositiveIntegerField(null=True, blank=True)
    
    # Attempt constraints (enforced at model level)
    ...
    # Academic flags (enforced at model level)
    is_timed = models.BooleanField(default=False)
    requires_lockdown = models.BooleanField(default=False)  # For exams
    ...
    def clean(self):
        ...
        if self.assignment_type == 'PROJECT':
            if self.max_attempts < 3:
                self.max_attempts = 3  # Projects allow revisions
